...
public class LoadMaintenanceJob extends QuartzJobBean {

	private final static Logger LOGGER = LoggerFactory.getLogger(LoadMaintenanceJob.class);
    ...
    protected void executeInternal(@NotNull JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            LOGGER.info("Call PagoPA to get maintenance");

            StationMaintenanceResponse response = pagoPaClient.maintenance(new URI(applicationProperties.getPagoPaClient().getMaintenance().getUrl()));
            LOGGER.info("Maintenance: {}", response);
...
//                    taxonomyDTO.setValidityEndDate(parseDate(taxonomy.get(TaxonomyField.VALIDITY_END_DATE.field), taxonomy.get(TaxonomyField.TAKINGS_IDENTIFIER.field)));
//
