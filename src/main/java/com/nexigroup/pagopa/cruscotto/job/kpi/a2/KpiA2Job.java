...
public class KpiA2Job extends QuartzJobBean {
    ...
    protected void executeInternal(@NotNull JobExecutionContext context) {
        ...
        if (instanceDTOS.isEmpty()) {
    		...
    		Set<String> taxonomyTakingsIdentifierSet = new HashSet<>(taxonomyService.getAllTakingsIdentifiers());
            ...
            instanceDTOS.forEach(instanceDTO -> {
            	...
            	try {
	                ...
	                int kpiA2AnalyticRecordsDataDeleted = kpiA2AnalyticDataService.deleteAllByInstanceModule(instanceModuleDTO.getId());
	                ...
	                AtomicReference<KpiA2ResultDTO> kpiA2ResultRef = new AtomicReference<>();
	                ...
	                kpiA2ResultRef.set(kpiA2ResultService.save(kpiA2ResultDTO));
                	...
                	List<KpiA2AnalyticDataDTO> kpiA2AnalyticDataDTOS = new ArrayList<>();                                
                  	...
                  	instanceDTO.getAnalysisPeriodStartDate()
                  			   ...
                  			   .forEach(date -> {
                  				   ...
                  				   List<PagoPaTaxonomyAggregatePositionDTO> pagoPaTaxonomyAggregatePositionDTOS =
                  						   pagoPaTaxonomyAggregatePositionService.findAllRecordIntoDayForPartner(instanceDTO.getPartnerFiscalCode(), date);
                  				   ...
                  				   for (PagoPaTaxonomyAggregatePositionDTO pagoPaTaxonomyAggregatePositionDTO : pagoPaTaxonomyAggregatePositionDTOS) {
                  					   ...
                  					   if(!isCorrectPayment(pagoPaTaxonomyAggregatePositionDTO.getTransferCategory(), taxonomyTakingsIdentifierSet)) {
                  						 sumIncorrectPaymentsDaily = sumIncorrectPaymentsDaily + pagoPaTaxonomyAggregatePositionDTO.getTotal();
                  					   }
                  				   }
                  	...
                  	double errorPercentagePeriod = totPaymentsPeriodValue.compareTo(0L) > 0 ? (double) (totIncorrectPaymentsPeriod.get() * 100) / totPaymentsPeriodValue : 0.0;
                    ...
                    kpiA2DetailResultDTO.setKpiA2ResultId(kpiA2ResultRef.get().getId());
                    ...
                    kpiA2ResultService.updateKpiA2ResultOutcome(kpiA2ResultRef.get().getId(), outcomeStatus);	                    
					...
					LOGGER.error("Error in elaboration instance {} for partner {} - {} with period {} - {}",
							...
							instanceDTO.getPartnerName(),
							instanceDTO.getAnalysisPeriodStartDate(),
							instanceDTO.getAnalysisPeriodEndDate(), e);
				}
            });
        }
        LOGGER.info("End");
    ...
    private boolean isCorrectPayment(String transferCategory, Set<String> taxonomyTakingsIdentifierSet) {
    	...
    	return taxonomyTakingsIdentifierSet.contains(taxonomyTakingsIdentifier);

//		for (String taxonomyTakingsIdentifier : taxonomyTakingsIdentifierList) {
//			if (transferCategory.equals(taxonomyTakingsIdentifier)) {
//				return true;
//			}
//		}
    }
	...
	public static void main(String[] args) {
		String input = "707091*"; // Sostituisci con la tua stringa

		String result;
		if (input.startsWith("9/")) {
			result = input.substring(0, 12);
		} else {
			result = "9/" + input.substring(0, 10);
		}
